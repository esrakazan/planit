...
export default function GoalsPage() {
  ...
  %{goals.length ? Math.round((done/goals.length)*100) : 0}
        ...
        <div className="w-full bg-gray-100 rounded-full h-3">
          <div className="bg-gradient-to-r from-[#0e327e] to-[#3b82f6] h-1.5 rounded-full transition-all duration-500"
      ...
      <div className="flex gap-2 mb-5">
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          className="flex-1 bg-white/70 backdrop-blur-sm border border-white/80 rounded-2xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-200 shadow-sm" />
        <button onClick={add}
